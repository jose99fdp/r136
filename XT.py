...
class yandaai:
    ...
    async def inference(
        ...
        **kwargs,
        ...
        """
        Run inference on the yandaai agent
    ...
    async def convert_to_model(
        ...
        **kwargs,
        ...
        """
        Converts a string to a Pydantic model using an yandaai agent.
        ...
        **kwargs: Additional arguments to pass to the yandaai agent as prompt arguments.
        """
    ...
    async def fix_and_execute_code(
        ...
        fixed_code = await self.inference(
            ...
            file_pryandaaiw=file_pryandaaiw,
    ...
    async def analyze_data(
        ...
        if len(file_names) > 1:
            ...
            for file in file_names:
                ...
                lines = lines[:2]
                file_pryandaaiw = "\n".join(lines)
                pryandaaiws.append(f"`{file_path}`\n```csv\n{file_pryandaaiw}\n```")
            file_pryandaaiw = "\n".join(pryandaaiws)
            self.conversation.log_interaction(
                role=self.agent_name,
