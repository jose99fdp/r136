...
class Agent:
    def __init__(self, agent_name=None, user=DEFAULT_USER, ApiClient: yandaaiSDK = None):
        self.agent_name = agent_name if agent_name is not None else "yandaai"
    ...
    async def text_to_speech(self, text: str):
        if self.TTS_PROVIDER is not None:
            ...
            output_url = f"{yandaai_uri}/outputs/{self.agent_id}/{filename}"
            return output_url
        ...
        if config_key == "commands":
            for command_name, enabled in new_config.items():
                ...
                if not command:
                    ...
                    if chain:
                        # Find or create the yandaai Chains extension
                        extension = (
                            session.query(Extension)
                            .filter_by(name="yandaai Chains")
                            .first()
                        )
                        if not extension:
                            extension = Extension(name="yandaai Chains")
