...
def execute_python_code(
    ...
    docker_image = "joshxt/safeexecute:main"
    docker_working_dir = f"/yandaai/WORKSPACE/{agent_id}/{conversation_id}"
    os.makedirs(docker_working_dir, exist_ok=True)
    host_working_dir = os.getenv("WORKING_DIRECTORY", "/yandaai/WORKSPACE")
...
class yandaai_actions(Extensions):
    """
    The yandaai Actions extension contains commands that allow the yandaai Agents to work with other agents and use pre-built chains to complete tasks.
    """
    ...
    def __init__(self, **kwargs):
        self.commands = {
            ...
            "Get CSV Pryandaaiw": self.get_csv_pryandaaiw,
            "Get CSV Pryandaaiw Text": self.get_csv_pryandaaiw_text,
    ...
    async def schedule_follow_up(
        ...
        title_pryandaaiw = title.split("\n")[0][:50] + ("..." if len(title) > 50 else "")

        # Create the follow-up task
        task_id = await task_manager.create_task(
            title=title_pryandaaiw,
    ...
    async def replace_init_in_file(self, filename: str, new_init: str):
        ...
        return f"{yandaai_uri}/outputs/extensions/{filename}"
    ...
    async def generate_openapi_chain(
        ...
        """
        Generate an yandaai extension from an OpenAPI JSON URL
...
"""
    ...
    async def get_csv_pryandaaiw(self, filename: str):
        """
        Get a pryandaaiw of a CSV file

        Args:
        filename (str): The filename

        Returns:
        str: The pryandaaiw of the CSV file consisting of the first 2-5 lines
        """
    ...
    async def get_csv_pryandaaiw_text(self, text: str):
        """
        Get a pryandaaiw of a CSV text

        Args:
        text (str): The text

        Returns:
        str: The pryandaaiw of the CSV text consisting of the first 2-5 lines
        """
        lines = text.split("\n")
        if len(lines) > 5:
            lines = lines[:5]
    ...
    async def convert_string_to_pydantic_model(
        ...
        response = self.ApiClient.prompt_agent(
            ...
            prompt_args={
                ...
                "schema": schema,
                "conversation_name": "yandaai Terminal",
            },
        )
