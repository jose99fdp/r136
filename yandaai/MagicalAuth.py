...
- yandaai_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- yandaai_URI: URL to the yandaai server
...
def is_yandaai_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("yandaai_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    yandaai_API_KEY = getenv("yandaai_API_KEY")
    authorization = str(authorization).replace("Bearer ", "").replace("bearer ", "")
    if yandaai_API_KEY:
        if authorization == yandaai_API_KEY:
        ...
        logging.error(
            "yandaai API Key is missing. Please set the yandaai_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("yandaai_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                yandaai = yandaaiSDK(base_uri=getenv("yandaai_URI"))
                yandaai.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                yandaai.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                yandaai.add_agent(
                    agent_name="yandaai",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        yandaai = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> yandaaiSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> yandaaiSDK:
        if not company_id:
            company_id = self.company_id
        ...
        yandaai.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return yandaai
