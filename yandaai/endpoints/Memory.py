...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await yandaai.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        yandaai = auth.get_company_agent_session(company_id=external_source.company_id)
        response = yandaai.delete_memory_external_source(
            agent_name="yandaai",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    yandaai = auth.get_company_agent_session(company_id=company_id)
    response = yandaai.learn_file(
        agent_name="yandaai",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    yandaai = auth.get_company_agent_session(company_id=company_id)
    response = yandaai.get_memories_external_sources(
        agent_name="yandaai",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hyandaain feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = yandaai.agent_interactions.agent_memory
    else:
        memory = yandaai.agent_interactions.agent_memory
    reflection = await yandaai.inference(
