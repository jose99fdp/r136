...
class TaskPlanInput:
    ...
    conversation_name: str = "yandaai Task Planning"
...
class CompanyCreateInput:
    ...
    agent_name: Optional[str] = "yandaai"
...
class Mutation:
    ...
    async def learn_file(self, info, agent_name: str, input: FileMemoryInput) -> bool:
        ...
        if input.company_id:
            yandaai = auth_manager.get_company_agent_session(company_id=input.company_id)
    ...
    async def submit_feedback(
        ...
        conversation = yandaai.conversation
        if conversation.has_received_feedback(message=input.message):
            return False

        memory = (
            yandaai.agent_interactions.agent_memory
            if input.positive
            else yandaai.agent_interactions.agent_memory
        )
...
"""
    ...
    async def run_chain_step(
        ...
        chain_step_response = await yandaai(
            user=user,
            agent_name=agent_name,
            api_key=auth,
            conversation_name=input.conversation_name,
