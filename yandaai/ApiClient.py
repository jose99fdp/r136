...
def verify_api_key(authorization: str = Header(None)):
    ...
    if yandaai_API_KEY:
        ...
        authorization = str(authorization).replace("Bearer ", "").replace("bearer ", "")
        if yandaai_API_KEY == authorization:
            return DEFAULT_USER
        if USING_JWT:
            try:
                token = jwt.decode(
...
def get_api_client(authorization: str = Header(None)):
    authorization = str(authorization).replace("Bearer ", "").replace("bearer ", "")
    return yandaaiSDK(base_uri="http://localhost:7437", api_key=authorization)
...
def is_admin(email: str = "USER", api_key: str = None):
    ...
    if email == "" or email is None or email == "None":
        ...
        if email == "" or email is None or email == "None":
            email = "USER"
    return is_yandaai_admin(email=email, api_key=api_key)
    return False
