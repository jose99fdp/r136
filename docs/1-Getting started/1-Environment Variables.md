...
The script sets up various environment variables for yandaai. Some key variables and their purposes include:

- `yandaai_API_KEY`: API key for yandaai (**automatically generated if not provided**)
- `yandaai_URI`: URI for the yandaai API
- `yandaai_BRANCH`: yandaai version to use (`stable` or `dev`)
- `yandaai_AUTO_UPDATE`: Whether to automatically update yandaai
- `yandaai_RLHF`: Enable reinforcement learning from hyandaain feedback (thumbs up/down buttons in UI)
- `yandaai_SHOW_SELECTION`: Controls which dropdowns are shown in the UI. Comma separated values. (default is `agent,conversation`)
