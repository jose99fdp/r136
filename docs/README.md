...
```bash
python start.py --yandaai-branch dev --yandaai-auto-update true --with-ezlocalai true
```
...
20. `--create-agent-on-register`: Create an agent named from your `yandaai_AGENT` environment variable if it is different than `yandaai` using settings from `default_agent.json` if defined (default: `true`)
21. `--create-yandaai-agent`: Create an agent called `yandaai` and trains it on the yandaai documentation upon user registration (default: `true`)
...
36. `--database-name`: Set the database name (default: `models/yandaai`)
...
41. `--yandaai-auto-update`: Enable or disable auto-updates (default: `true`)
...
Need more information? Check out the documentation for more details to get a better understanding of the concepts and features of yandaai.
